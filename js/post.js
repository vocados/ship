...
function callAjax(c, b) {
  ...
  function getLocation(end) {
    var c = "/timetable/"+encodeURI(start + ' 에서 '+end+' 가는 여객선 시간표')+"/";
    location.href = c;

  }
  function detail(a) {
    callAjax("/json/"+a+".json", function(arr){
        var resultArr = [];
        for (var i=0; i<arr.length; i++) {
          var row = arr[i];
          resultArr.push('<a href="javascript:;" onclick="detail2(\''+row+'\');" class="list-group-item list-group-item-action">'+row+'</a>');
        }
...
function detail2(a) {
	start = a;
	if (!loaded) {
		document.getElementById('master').innerHTML =  document.getElementById('detail').innerHTML;
		loaded = true;
	}
    ...
    $(document).ready(function(){
      ...
      myModalEl.addEventListener('show.bs.modal', function (event) {
        callAjax("/json/master.json", function(arr){
          ...
          for (var i=0; i<arr.length; i++) {
            var row = arr[i];
            resultArr.push('<a href="javascript:;" onclick="detail(\''+row['area_cd']+'\');" class="list-group-item list-group-item-action">'+row['parent']+'</a>');
          }
